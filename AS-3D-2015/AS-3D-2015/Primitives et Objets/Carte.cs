...
{
    public class Carte : Microsoft.Xna.Framework.GameComponent, IGame
    {
        ...
        public Carte(Game jeu, Vector3 étendue, string nomCarteTerrain, Lumière lumièreJeu)
            : base(jeu)
        {
            Étendue = étendue;
        ...
        private void CréerTableauPoints()
        {
            ...
            for (int j = 0; j < NbRangées; ++j)
            {
                for (int i = 0; i < NbColonnes; ++i)
                {
                    ...
                    {
                        Game.Components.Add(new TuileÉclairée(Game, new Vector3(PtsSommets[i, j].X, PtsSommets[i, j].Y, PtsSommets[i, j].Z), Delta, "Floor", true, "SpotLight", LumièreJeu));
                        Game.Components.Add(new TuileÉclairée(Game,
                                                new Vector3(PtsSommets[i, j].X, PtsSommets[i, j].Y + 1.5f * Delta.Y, PtsSommets[i, j].Z), Delta, "Ceiling", false, "SpotLight", LumièreJeu));
                    }
                    else
                    {
                        Game.Components.Add(new CubeColoréÉclairé(Game,
                                               new Vector3(PtsSommets[i, j].X, PtsSommets[i, j].Y, PtsSommets[i, j].Z), new Vector3(Delta.X, Delta.Y, Delta.Z), "Wall","SpotLight",LumièreJeu));
                        Game.Components.Add(new CubeColoréÉclairé(Game,
                                               new Vector3(PtsSommets[i, j].X, PtsSommets[i, j].Y + Delta.Y, PtsSommets[i, j].Z), new Vector3(Delta.X, Delta.Y, Delta.Z), "Wall","SpotLight",LumièreJeu));
                    }
                    ++k;
                }
            }
